...
function assert(condition, message) {
  stepCount++;
  if (!condition) {
    console.error(`  FAIL [${stepCount}] ${message}`);
    throw new Error(message);
  }
  passCount++;
  console.log(`  ok   [${stepCount}] ${message}`);
}

async function request(method, path, { body, expectedStatus }) {
  console.log(`\n  --> ${method} ${path}  (expect ${expectedStatus})`);
...
async function main() {
  ...
  assert(true, "DELETE /books/:id must return 204");

  console.log(`\n[Books - reviews & average rating]`);
  const existingList = await request("GET", "/books?page=1&limit=1", {
    expectedStatus: 200,
  });
  assert(
    Array.isArray(existingList?.data) && existingList.data.length > 0,
    "GET /books must return at least one book for review tests",
  );
  const existingBookId = existingList.data[0].id;

  const createdReview = await request(
    "POST",
    `/books/${existingBookId}/reviews`,
    {
      expectedStatus: 201,
      body: {
  ...
  const reviews = await request(
    "GET",
    `/books/${existingBookId}/reviews?page=1&limit=5`,
    {
      expectedStatus: 200,
    },
  );
  ...
  const avg = await request("GET", `/books/${existingBookId}/average-rating`, {
    expectedStatus: 200,
  });
  assert(
  ...
  console.log(`\n=== PASS: ${passCount}/${stepCount} checks passed ===\n`);
}
